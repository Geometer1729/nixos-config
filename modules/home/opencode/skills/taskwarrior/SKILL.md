...
Read `~/.taskopenrc` to resolve the note path template in `notes.command`. Use
the task's full UUID for `$UUID`. The interactive opener is `taskopen <UUID>`;
agents should read and write the Markdown file directly.

Notes do not need a `Notes` annotation: `no_annotation_hook` opens the same
file for tasks without one. Do not add note annotations; they only clutter
reports with a `[1]` count.
...
4. Read back the note. Report the task ID, UUID, and note path.
...
3. Read the resolved Markdown path using its full UUID; a missing annotation
   says nothing about whether a note exists. If it contains `[Redirect](path#anchor)`, read the linked document's relevant
