...
-- My fork of the same module from xmonad-contrib

module NamedScratchpad (
  -- * Usage
  -- $usage
  NamedScratchpad (..),
  NspAction (..),
...
data NspAction = Toggle | Set Bool
...
namedScratchpadAction ::
  ...
  String ->
  X ()
namedScratchpadAction = flip <$> customRunNamedScratchpadAction runApplication
...
someNamedScratchpadAction ::
  ...
  NspAction ->
  X ()
someNamedScratchpadAction f runApp _ns scratchpadName action = do
    ...
    Just conf -> withWindowSet $ \winSet -> do
      ...
      case NE.nonEmpty matchingOnCurrent of
        ...
        Nothing -> case NE.nonEmpty matchingOnAll of
          Nothing -> runApp conf
          Just wins -> when canFocus $ f (windows . W.shiftWin (W.currentTag winSet)) wins
        -- matching window running on current workspace -> window should be shifted to scratchpad workspace
        Just wins -> when canHide $ shiftToNSP (W.workspaces winSet) (`f` wins)
    Nothing -> return ()
 where
  canFocus = action `elem` [Toggle, Set True]
  canHide = action `elem` [Toggle, Set False]

withC :: NamedScratchpads -> (Maybe String -> X ()) -> X ()
withC _nsps = undefined
...
dynamicNSPAction :: String -> NspAction -> X ()
